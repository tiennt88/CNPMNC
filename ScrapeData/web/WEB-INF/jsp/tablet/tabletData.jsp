...
<script>
    ...
    page.getIndex = function (index) {
        var grid = dijit.byId('tabletDataListGird');
        return grid.currentRow + index + 1;
    }

</script>

<sd:TitlePane id="tabletData"  key="Thông tin">
    <s:form  id="tabletDataForm"  name="tabletDataForm"  method="POST">
        <table width="100%">
            ...
            <tr>
                <td>
                    <sd:CheckBox id="approve" checked="false" name="tablet.approve" key="Chưa Duyệt"/>
                </td>
                <td>
                    <sd:CheckBox id="today" checked="false" name="tablet.today" key="today"/>
                </td>
                <td></td>
            </tr>
            <tr>
                <td colspan ="3" align="center">
                    ...
                    <sd:Button id="tabletAnalysis" key="Analysis"
                               onclick="page.tabletAnalysis()">
...
<sd:TitlePane key="Dữ Liệu" id="tabletDataList">
    <form id="tabletDataListForm" name="tabletDataListForm">
        <div id="tabletDataListDiv" >
            <sd:DataGrid clientSort="true"   getDataUrl="tabletDataSearch!onSearch.do" id="tabletDataListGird" style="width: 100%; height: 100%;" rowsPerPage="10">
...
<script>
    ...
    page.onSearch = function(){
        ...
        dijit.byId('tabletDataListGird').vtReload("tabletDataSearch!onSearch.do","tabletDataForm",null,callback) ;
    }
    ...
    dojo.connect(dijit.byId("tabletDataListGird"), "onCellClick", function (e) {
        var grid = dijit.byId("tabletDataListGird");
        ...
        if(colField == "_R_D_"){
            if(item){
                ...
                dijit.byId("brand").setAttribute('value', grid.getCell(6).get(idx, item).toUpperCase());
                ...
                var approve = grid.getCell(21).get(idx, item);
                if(approve == "Y"){
                    dijit.byId("approve").setAttribute('checked', false);
                }else{
            ...
            var xhrArgs = {
                ...
                load: function(data) {
                    notification(data.label,data.customInfo);
                    dijit.byId("tabletDataListGird").vtReload("tabletDataSearch!onSearch.do","tabletDataForm",null,page.showMessage) ;
                }
            };
            dojo.xhrGet(xhrArgs);
        }
...
//        }
    ...
    page.onEdit = function(){
        if(validate() == true){
            ...
            dijit.byId('tabletDataListGird').vtReload("tabletDataUpdate!onUpdate.do","tabletDataForm",null,callback) ;
            dijit.byId("edit").setAttribute('disabled', true);
        }
    }
    ...
    page.onDelete = function(){
        ...
        if(confirm("Bạn thực sự muốn xóa?")){
            ...
            dijit.byId('tabletDataListGird').vtReload("tabletDataDelete!onDelete.do","tabletDataForm",null,callback) ;
            dijit.byId("delete").setAttribute('disabled', true);
            page.reset();
        }
    ...
    page.tabletAnalysis = function(){

        var xhrArgs = {
            url: "tabletAnalysis.do",
            form: document.getElementById("tabletDataForm"),
            load: function(data) {
//                alert(data);//json tra ve tren JS o dang string. Can phai parse thanh doi tuong json tren JS
    ...
    page.exportExcel = function(){
        sd.connector.post("tabletDataExport!exportExcel.do","result","tabletDataForm");
    }

    page.importFile = function(){
        openPopup("upload.do?type=Tablet",500,170);
    }

    page.template = function(){
        <%
            ResourceBundle rb = ResourceBundle.getBundle("config");
            String template = rb.getString("template_file_path")+"TabletDataReport.xlsx";
