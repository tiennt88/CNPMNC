...
<script>
    ...
    page.getIndex = function (index) {
        var grid = dijit.byId('mobileRootListGird');
        return grid.currentRow + index + 1;
    }

</script>

<sd:TitlePane id="mobileRoot"  key="Dữ liệu Mobile FTG">
    <s:form  id="mobileRootForm"  name="mobileRootForm"  method="POST">
        <table width="100%">
            <s:hidden name="mobile.id" id="id"/>
            <s:hidden name="mobile.id1" id="id1"/>
            <tr>
                <td>
                    <sd:SelectBox id="brand" key="Hãng" name="mobile.brand" data="brands" valueField="value" labelField="name" required="true" />
                </td>
                <td>
                    <sd:TextBox id="itemCode" name="mobile.itemCode" key="Mã sản phẩm" trim="true" required="true" maxlength="50"/>
                </td>
                <td>
            ...
            <tr>
                ...
                <td>
                    <sd:TextBox id="partno"   name="mobile.partno" key="PartNo" trim="true" maxlength="50"/>
                </td>
                <td>
            ...
            <tr>
                ...
                <td>
                    <sd:TextBox id="color" name="mobile.color" key="color" trim="true"  maxlength="50"/>
                </td>
                <td>
                    <sd:CheckBox id="mapping" checked="false" name="mapping" key="Match"/>
                    <sd:CheckBox id="today" checked="false" name="mobile.today" key="today" cssStyle="margin-left:15%"/>
                </td>
            </tr>
            <tr>
...
<sd:TitlePane key="Danh sách sản phẩm laptop lấy dữ liệu" id="mobileRootList">
    <form id="mobileRootListForm" name="mobileRootListForm">
        <div id="mobileRootListDiv" >
            <sd:DataGrid clientSort="true"   getDataUrl="mobileRootSearch!onSearch.do" id="mobileRootListGird" style="width: 100%; height: 100%;" rowsPerPage="10">
...
//	notification('error','xin chao');
        ...
        dijit.byId('mobileRootListGird').vtReload("mobileRootSearch!onSearch.do","mobileRootForm",null,callback) ;
    }
    ...
    dojo.connect(dijit.byId("mobileRootListGird"), "onCellClick", function (e) {
        var grid = dijit.byId("mobileRootListGird");
        ...
        if(colField == "_R_D_"){
            if(item){
                ...
                document.getElementById("color").value = grid.getCell(21).get(idx, item);
        ...
        if(colField == "match"){
            openPopup("mobileComparePopUp.do?id="+grid.getCell(1).get(idx, item),1000,550);
...
//        }
    ...
    page.onEdit = function(){
        if(validate() == true){
            ...
            dijit.byId('mobileRootListGird').vtReload("mobileRootUpdate!onUpdate.do","mobileRootForm",null,callback) ;
            dijit.byId("edit").setAttribute('disabled', true);
        }
    }
    ...
    page.onDelete = function(){
        ...
        if(confirm("Bạn thực sự muốn xóa?")){
            ...
            dijit.byId('mobileRootListGird').vtReload("mobileRootDelete!onDelete.do","mobileRootForm",null,callback) ;
            dijit.byId("delete").setAttribute('disabled', true);
            page.reset();
        }
    }
    ...
    page.template = function(){
        <%
            ...
            String template = rb.getString("template_file_path")+"LaptopTemplate.xlsx";
...
//        openPopup("upload.do?type=Mobile",500,200);
//    }
