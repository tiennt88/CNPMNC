 ...
 * @author KeyLove
 */
public class MobileModelCompareDAO extends BaseHibernateDAOMDB{
    ...
    public String takeSQL(String type){
        ...
        sql.append(" f_camera frontCamera,os,battery,sim,PROMOTION,color,a.last_update lastUpdate from MOBILE_DATA a, MOBILE_CONFIGURATION b ");
        sql.append(" where a.ID = b.MOBILE_DATA_ID and a.WEB = '").append(rb.getString(Constans.WEB_ROOT)).append("' ");
        ...
        sql.append(" f_camera frontCamera, os,battery,sim,PROMOTION,color,c.last_update lastUpdate from  MOBILE_DATA c, MOBILE_CONFIGURATION d ");
        sql.append(" where c.ID = d.MOBILE_DATA_ID and c.WEB != '").append(rb.getString(Constans.WEB_ROOT)).append("' ");
        ...
        if(mobile != null){
            if( mobile.getStorage() != null && "on".equals(mobile.getStorage()) ){
                sql.append(" and isnull(x.STORAGE,'') = isnull(y.STORAGE,'') ");
            }
            if( mobile.getRam() != null && "on".equals(mobile.getRam()) ){
...
//            UserToken userToken = (UserToken) getRequest().getSession().getAttribute("userToken");
    ...
    public String exportExcel(){
        try{
            filename = "MobileCompareModel.xlsx";
...
//            String downFile = ServletActionContext.getServletContext().getRealPath(rb.getString(Constans.DOWNLOAD_FILE_PATH) + "MobileCompareModel_"+random.nextInt(1000)+".xlsx");
            String downFile = rb.getString(Constans.DOWNLOAD_FILE_PATH) + "MobileCompareModel_"+random.nextInt(1000)+".xlsx";
